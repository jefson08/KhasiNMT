...
def main():
    ...
    with open("config_translation_hybrid.yaml") as f:
        try:
            ...
            opt.model_path_s_sent = dict["model_path_s_sent"]
            opt.model_path_l_sent = dict["model_path_l_sent"]
            opt.kha_pretrain = dict["data"]["pre_train_model"]["model_path"]
            opt.s_context_embedding = int(dict["s_context_embedding"])
            opt.l_context_embedding = int(dict["l_context_embedding"])
            opt.no_of_words = int(dict["no_of_words"])
    ...
    if opt.s_context_embedding or opt.l_context_embedding:
        if opt.src_lang == 'kha':
    ...
    s_model, _, _, _ = get_model(opt, opt.src_tokenizer.vocab_size, opt.trg_tokenizer.vocab_size,
                                 opt.src_tokenizer.get_vocab()['<pad>'], opt.trg_tokenizer.get_vocab()['<pad>'], opt.lr, (0.9, 0.98), 1e-9, False, opt.model_path_s_sent)
    l_model, _, _, _ = get_model(opt, opt.src_tokenizer.vocab_size, opt.trg_tokenizer.vocab_size,
                                 opt.src_tokenizer.get_vocab()['<pad>'], opt.trg_tokenizer.get_vocab()['<pad>'], opt.lr, (0.9, 0.98), 1e-9, False, opt.model_path_l_sent)
        ...
        if opt.text == 'f':
            try:
                ...
                for opt.text in tqdm(process_txt):
                    no_words = len(opt.text.split())
                    if no_words <= opt.no_of_words:
                        opt.context_embedding = opt.s_context_embedding
                        phrase = translate(opt, s_model)
                    else:
                        opt.context_embedding = opt.l_context_embedding
                        phrase = translate(opt, l_model)
                    f.write(phrase+"\n")
                    i = i+1
                f.close()
                continue
